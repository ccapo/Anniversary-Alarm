...
void melody1() {
  ...
  for (uint8_t n = 1; n <= 2; n++) {
    ...
    note(NOTE_AS, 6); delay(dur8); note(0, 0);
    note(NOTE_D, 7); delay(dur4); note(0, 0);
    note(NOTE_D, 7); delay(dur4); note(0, 0);
    note(NOTE_AS, 6); delay(dur8); note(0, 0);
    note(NOTE_C, 7); delay(dur4); note(0, 0);
    note(NOTE_DS, 7); delay(dur4); note(0, 0);
    note(NOTE_D, 7); delay(dur4); note(0, 0);
    note(NOTE_AS, 6); delay(dur4); note(0, 0);
    note(NOTE_AS, 5); delay(dur8); note(0, 0);
    ...
    note(NOTE_A, 6); delay(dur4); note(0, 0);
    note(NOTE_C, 7); delay(dur4); note(0, 0);
    note(NOTE_AS, 6); delay(dur4); note(0, 0);
...
void loop() {
  ...
  // Retrieve birthdayYear from EEPROM
  birthdayYear = EEPROM.read(EEPROM_ADDRESS);
  if (birthdayYear <= 0 || birthdayYear >= 255) birthdayYear = 1;
  ...
  // Count out number of birthdays
  delay(250);
  for (byte i = 0; i < birthdayYear; i++) {
    note(NOTE_C, 6); delay(250); note(0, 0);
    delay(400);
  }

  // Update birthdayYear in EEPROM
  EEPROM.update(EEPROM_ADDRESS, ++birthdayYear);
  delay(10);
}
